...
def test_monthly_bars_keep_empty_months(client):
    """빈 달이 곧 관리 공백이라, 0인 달도 자리를 남긴다."""
    project = _make(client, "가끔 보고하는 과제")
    _freeze_report(client, project["id"], "2026-03-10")

    months = client.get("/api/home", params={"year": "2026"}).json()["monthly_reports"]
    assert len(months) == 12
    assert months[2] == {"month": 3, "count": 1}
