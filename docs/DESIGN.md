...
| 화면 | 내용 |
|---|---|
| **홈 (`#/`)** | 첫 화면 (TODO 56). 연도 하나를 골라 **"우리 팀이 그 해에 무엇을 했나"**를 본다 — 이번 주 할 일(보고 대상·기한 초과·마감 임박·오래 방치), 팀 현황(과제·진행중·완료·보고 횟수·효과 금액), 연도별 추이, **팀원별 성과**(ROADMAP R1)와 **속성별** — 둘 다 상태 여섯 칸을 `STATUSES` 순서로 세운다(0도 빼지 않는다. 줄끼리 세로로 견주는 자리라 칸이 줄마다 달라지면 비교가 안 된다) —, **이번 주 할 일과 팀 현황은 나란히 선다** — 세로로 쌓으면 오른쪽이 비고 매일 보는 것을 보려고 스크롤해야 한다 (1100px 아래에서는 한 줄씩). 모든 수는 눌러서 그 조건의 목록으로 이어진다 (5.8). 대시보드와 목적이 다르다 — 대시보드는 *지금 무엇을 볼까*, 홈은 *올해 무엇을 했나* |
