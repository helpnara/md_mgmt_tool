...
> 이름은 2026-09-24 에 정해졌다 (TODO 132). 나무가 한 해에 한 겹씩 굵어지듯,
> 이 도구도 주 단위로 한 줄씩 쌓여야 그 해가 보인다.
...
| 과제 속성 | 스마트과제 / R&D / 투자 / 기획보고 / 국책과제 / 유지보수 (과제당 하나, **설정에서 더하고 빼고 고친다**) |
...
due_date: 2026-12-20
completed_at:              # 상태가 done 이 되는 순간 자동으로 — 손으로 적은 날이 이긴다 (TODO 104)
...
feedback: |                    # 보고 자리에서 받은 지시 — 확정 뒤에도 쓸 수 있는 유일한 본문 (TODO 107)
  원가 근거 자료를 다음 주까지 보완할 것
feedback_done:                 # 다음 보고에서 답한 날. 비어 있으면 아직 답하지 않은 지시다
...
* `feedback` 만은 확정 뒤에도 쓴다 — 보고 본문은 "그때 무엇을 보고했는가"라 굳지만, "그 자리에서 무슨 말을 들었는가"는 보고가 끝난 뒤에야 적을 수 있다 (5.15).
...
> 이 기능을 넣다가 검색의 구멍 하나를 함께 막았다. 두 글자로 찾으면 FTS5 trigram
> (세 글자부터) 대신 LIKE 로 물러나는데, 그 LIKE 가 **제목·본문만** 훑고 있었다.
> 그래서 `"구매"` 같은 부서나 `"이준"` 같은 두 글자 이름이 짧다는 이유만으로 안 찾혔다.
> **두 길이 같은 자리를 봐야 한다** — LIKE 도 담당자·태그·유관부서를 함께 훑는다.
...
CREATE TABLE project (
  ...
  start_date    TEXT,
  due_date      TEXT,
  completed_at  TEXT,                    -- 완료일 (TODO 104, 스키마 11). 홈의 "그해 끝낸 과제"가 이 열을 센다
  created_at    TEXT,
...
CREATE TABLE report (
  ...
  feedback      TEXT,                    -- 보고 뒤에 받은 지시 (TODO 107, 스키마 11)
  feedback_done TEXT,                    -- 답한 날. NULL 이고 feedback 이 있으면 "답하지 않은 지시"
  file_mtime    REAL,
  UNIQUE(project_id, rel_path)
);
...
* **말한 것과 데려가는 곳이 같아야 한다** (TODO 91). 수를 정확히 적는 것만으로는 모자란다.
  ...
  개수만으로는 갈 곳을 만들 수 없다.
* **한 줄에 갈 곳이 둘이면, 넓은 쪽이 주된 길이어야 한다** (TODO 130). *확정을 기다리는
  초안* 은 한동안 과제명(넓다)은 과제 상세로, 오른쪽 [초안 열기](작다)는 초안으로 갔다.
  그 카드에 온 사람이 원하는 곳은 거의 언제나 초안인데, 누르기 쉬운 쪽이 다른 데로 갔다.
  지금은 **줄 전체가 초안으로 가는 `<a>`** 다. 보고 편집기는 과제 상세 화면 *안에서* 열리므로
  과제를 훑어보는 길도 함께 간다. `<li onClick>` 이 아니라 `<a>` 를 줄만큼 넓히는 것이 규칙이다
  — 키보드로 닿고, 가운데 단추로 새 탭에 열리고, 주소가 미리 보인다.
* **줄이 여럿인 목록의 칸은 격자로 고정한다** (TODO 131). flex + `space-between` 으로 두면
  칸의 자리가 **제 왼쪽 글자 길이를 따라** 움직여, 날짜가 줄마다 다른 곳에 선다. 세로로
  견주는 자리(날짜·수)가 있으면 격자로 폭을 못 박고, 넘치는 글자는 제 칸 안에서 말줄임한다.
  좁은 화면에서는 칸을 줄이는 대신 **줄을 접는다.**
...
### 돌아갈 곳도 주소가 든다 (TODO 127)

`back` 에 **온 곳**을 실어 보내고, 도착한 화면이 그것으로 `← 홈` 을 그린다. 여기서 두 가지가
걸렸다.

* **홈의 주소는 `#/` 라 빈 글자가 된다.** 그것을 그대로 실으면 *"온 곳을 모른다"* 와 구분되지
  않아 홈으로 돌아갈 수 없다. 그래서 홈만 `home` 이라는 이름을 따로 쓴다(`HOME_BACK`).
  값이 없는 것과 "없음" 을 뜻하는 값은 다르다 — 설정의 `null`/`[]` 과 같은 갈래다(4.8).
* **주소를 만드는 자리가 둘이면 한쪽만 고쳐진다.** `listLink` 가 홈과 과제 상세에 따로 있었고
  둘 다 `back` 을 안 실었다. `nav.ts` 한 곳으로 모아 `projectLink` 와 같은 규칙을 쓰게 했고,
  그다음 `screenLink(화면, 조건)` 으로 넓혔다 (TODO 128) — **지금 홈에는 손으로 쓴 주소가
  하나도 없다.** 링크를 새로 붙여도 돌아갈 길이 저절로 따라온다.

도착한 화면은 **`back` 을 자기 상태에 넣는다.** 조건을 바꿀 때마다 주소를 다시 쓰기 때문에
(`useAddressBar`), 상태에 없으면 거르는 순간 사라진다. 다만 **서버로는 보내지 않는다** —
화면의 것이지 조건이 아니다.

`back` 이 없으면 그 줄을 **그리지 않는다.** 메뉴로 들어온 화면에 가짜 뒤로 가기를 만들면
누를 때마다 엉뚱한 곳으로 간다 — 없는 길을 만들지 않는다.
...
## 5.15 보고 뒤에 남는 것 — 지시사항 · 완료일 · 기간 (TODO 104 · 107 · 110)

2026-09-09 의 [1인 팀장 관점 전수 검토](검토-1인팀장-2026-09-09.md)에서 나온 셋. 셋 다
**"보고가 끝난 뒤"** 에 관한 것이다 — 그동안 이 도구는 보고를 *만드는 데까지*만 있었다.

### 지시사항은 확정된 보고에 적는다

보고 문서는 확정되면 굳는다(4.4). 그런데 지시는 **보고가 끝난 뒤에** 온다. 둘을 한 본문에
두면 "굳은 문서를 고친다"가 되고, 진행일지에 쓰면 다른 기록에 섞인다. 그래서 확정된 보고에
**본문과 따로** `feedback` 한 칸을 두고, 그 칸만 확정 뒤에도 쓴다.

지시가 하는 일은 셋이고, 셋 다 **사용자가 찾아가지 않아도** 일어난다.

| 어디서 | 무엇 |
|---|---|
| 과제 상세 위쪽 | *답하지 않은 지시 N건* — 번호 연도 안내(`YearFixNote`)와 같은 무게의 띠 |
| 다음 보고 초안 | 맨 위에 `## 지난 보고 지시사항` 으로 딸려 들어간다 — 초안이 먼저 그것을 물어본다 |
| 보고 이력 | `feedback=open` 으로 거른다 |

닫는 것은 사람이 한다(**[답변함]** → `feedback_done` 에 날짜). 초안에 딸려 들어갔다고
저절로 닫지 않는다 — 초안에 들어간 것과 *답한 것*은 다르다.

### 완료일은 두 기준을 나란히 둔다

홈의 완료 수는 **과제 번호의 연도**로 셌다(5.8). 그러면 2025 년에 시작해 2026 년에 끝낸
과제는 2026 년 성과에 잡히지 않는다. 그렇다고 완료일 기준으로 *바꾸면* 지금까지의 수가
달라진다. 그래서 **바꾸지 않고 하나를 더 세운다** — 번호 연도 기준 [완료]와 완료일 기준
[그해 끝낸 과제]. 어느 쪽이 "성과"인지는 사용자가 정할 일이고, 이 도구가 지금 할 일은
**날짜가 남게 하는 것**이다. 소급이 안 되는 것은 순서와 무관하게 먼저 넣는다는 원칙
(향후계획 1순위)의 실례다.

`completed_at` 은 상태가 `done` 이 되는 순간 자동으로 붙고, 벗어나면 비워지며, 손으로 적은
날이 이긴다. 상태가 바뀔 때는 진행일지에 `(상태) 진행중 → 완료` 한 줄이 함께 남는다
(TODO 105) — 파일이 원본이라는 원칙에 맞고, 다음 보고 초안에 그 사실이 저절로 든다.

### 기간은 날짜가 있는 숫자만 자른다

기준 연도 옆의 반기·분기는 **보고 횟수 · 그해 끝낸 과제 · 역량 이력**만 따른다. 과제 수와
