# 과제 이력 관리 도구 — 설계 문서 (v0.4)

작성일: 2026-09-01 · 상태: **M0~M3 구현 완료, M4 진행 예정**
변경 이력
...
| **M4. 보고 이력 & 보고 예측** ◀ 다음 | 보고 초안 자동 생성 → 정리 → 확정(스냅샷), 보고 자료(xlsx) 첨부·미리보기, 보고 이력 탭, 보고 대상 후보 대시보드, 엑셀 붙여넣기용 복사 | 매주 "어느 과제를 보고할지"를 도구가 근거(경과일·미보고 건수)와 함께 제시하고, 과거 보고 문서를 그 자리에서 다시 열 수 있다 |
