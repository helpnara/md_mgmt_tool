...
| 과제 상태 | 기획보고 / 제안 / 검토 / 진행중 / 보류 / 완료 / 중단 (7단계) |
...
저장은 영문 키로, 화면 표시는 한글 라벨로 한다. 나중에 라벨을 바꿔도 파일을 다시 쓸 필요가 없다.
...
* 보드 뷰의 칸 순서는 위 순서를 그대로 따른다. `완료`·`중단`은 기본 접힘.
* 상태 목록은 `config.py`의 상수 하나로 관리하여, 단계가 늘거나 라벨이 바뀔 때 한 곳만 고치면 되게 한다.
